...
class SlackChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        # Load the bot user ID for mention checks.
        try:
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            # Update reactions on the final reply.
    ...
    async def _on_socket_request(
        ...
        # Only handle plain messages and mentions.
        ...
        # Skip bot and system events.
        if event.get("subtype"):
            return
        if self._bot_user_id and sender_id == self._bot_user_id:
            return

        # Slack may send both `message` and `app_mention` for one mention.
        text = event.get("text") or ""
        if event_type == "message" and self._bot_user_id and f"<@{self._bot_user_id}>" in text:
            return

        # Log the event shape for debugging.
        logger.debug(
        ...
        # Add the in-progress reaction when possible.
        try:
        ...
        # Use a thread-scoped session key for channels.
    ...
    async def _update_react_emoji(self, chat_id: str, ts: str | None) -> None:
        """Replace the progress reaction with the done reaction."""
    ...
    def _fixup_mrkdwn(cls, text: str) -> str:
        """Clean up markdown that slackify_markdown misses."""
    ...
    def _convert_table(match: re.Match) -> str:
        """Convert a Markdown table to plain Slack text."""
