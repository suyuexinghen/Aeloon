...
def _render_table_box(table_lines: list[str]) -> str:
    """Render a markdown table for <pre> output."""
...
def _markdown_to_telegram_html(text: str) -> str:
    ...
    # 8. Convert italic text.
    text = re.sub(r"(?<![a-zA-Z0-9])_([^_]+)_(?![a-zA-Z0-9])", r"<i>\1</i>", text)

    # 9. Convert strikethrough.
    text = re.sub(r"~~(.+?)~~", r"<s>\1</s>", text)

    # 10. Convert bullet lists.
...
class TelegramChannel(BaseChannel):
    ...
    BOT_COMMANDS = [
        ...
        BotCommand("status", "Show channel status"),
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        self._chat_ids: dict[str, int] = {}  # sender_id -> chat_id
    ...
    def is_allowed(self, sender_id: str) -> bool:
        """Keep Telegram's legacy id|username allowlist match."""
    ...
    async def start(self) -> None:
        ...
        # Use a larger connection pool for long runs.
        req = HTTPXRequest(
        ...
        # Load bot info and register the command menu.
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Only stop typing on final responses.
        ...
        if msg.content and msg.content != "[empty message]":
            ...
            for chunk in split_message(msg.content, TELEGRAM_MAX_MESSAGE_LEN):
                # Final responses simulate streaming first.
                if not is_progress:
    ...
    async def _send_with_streaming(
        ...
        """Simulate streaming before sending the final message."""
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm aeloon.\n\n"
            "Send me a message and I'll respond!\n"
            "Type /help to see available commands."
        )

    async def _on_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle /help without ACL checks."""
    ...
    def _sender_id(user) -> str:
        """Build a sender_id that keeps username matching."""
        sid = str(user.id)
        return f"{sid}|{user.username}" if user.username else sid

    @staticmethod
    def _derive_topic_session_key(message) -> str | None:
        """Build a topic-scoped session key."""
    ...
    def _build_message_metadata(message, user) -> dict:
        """Build common Telegram metadata."""
    ...
    async def _ensure_bot_identity(self) -> tuple[int | None, str | None]:
        """Load bot identity once for mention checks."""
    ...
    def _has_mention_entity(
        ...
        """Check whether mention entities target the bot."""
    ...
    async def _is_group_message_for_bot(self, message) -> bool:
        """Check whether a group message should be handled."""
    ...
    def _remember_thread_context(self, message) -> None:
        """Cache thread IDs for follow-up replies."""
        message_thread_id = getattr(message, "message_thread_id", None)
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Handle inbound Telegram messages."""
        if not update.message or not update.effective_user:
            return
        ...
        # Save chat_id for replies.
        self._chat_ids[sender_id] = chat_id

        if not await self._is_group_message_for_bot(message):
            return

        # Build content from text and media.
        content_parts = []
        media_paths = []
        ...
        # Buffer media groups and forward them as one turn.
    ...
    async def _flush_media_group(self, key: str) -> None:
        """Flush one buffered media group."""
        try:
    ...
    async def _typing_loop(self, chat_id: str) -> None:
        """Keep sending the typing action until cancelled."""
        try:
            while self._app:
    ...
    async def _on_error(self, update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Log polling and handler errors."""
    ...
    def _get_extension(
        ...
        """Pick a file extension for downloaded media."""
        if mime_type:
            ext_map = {
                "image/jpeg": ".jpg",
                "image/png": ".png",
