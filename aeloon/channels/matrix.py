...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install aeloon-ai[matrix]"
    ) from e
...
from aeloon.core.config.paths import get_data_dir, get_media_dir
from aeloon.core.config.schema import Base
from aeloon.utils.helpers import safe_filename

TYPING_NOTICE_TIMEOUT_MS = 30_000
# Keep this below the typing timeout.
...
def _filter_matrix_html_attribute(tag: str, attr: str, value: str) -> str | None:
    """Filter HTML attributes to a safe Matrix subset."""
...
class _NioLoguruHandler(logging.Handler):
    """Route matrix-nio logs into Loguru."""
...
def _configure_nio_logging_bridge() -> None:
    """Bridge matrix-nio logs to Loguru."""
...
class MatrixChannel(BaseChannel):
    """Matrix channel using sync polling."""
    ...
    def _collect_outbound_media_candidates(self, media: list[str]) -> list[Path]:
        """Resolve outbound attachment paths."""
        seen: set[str] = set()
    ...
    def _build_outbound_attachment_content(
        *,
        ...
        """Build Matrix content for an uploaded attachment."""
        prefix = mime.split("/")[0]
    ...
    async def _resolve_server_upload_limit_bytes(self) -> int | None:
        """Query the homeserver upload limit once."""
    ...
    async def _effective_media_limit_bytes(self) -> int:
        """Return the effective media upload limit."""
    ...
    async def _upload_and_send_attachment(
        ...
        """Upload and send one attachment."""
        if not self.client:
    ...
    async def _start_typing_keepalive(self, room_id: str) -> None:
        """Start typing keepalive updates."""
        await self._stop_typing_keepalive(room_id, clear_typing=False)
    ...
    async def _fetch_media_attachment(
        ...
        """Download and save a Matrix attachment."""
