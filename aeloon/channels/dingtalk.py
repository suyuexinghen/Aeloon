...
try:
    ...
    # Keep class definitions importable when the SDK is missing.
...
class AeloonDingTalkHandler(CallbackHandler):
    """Stream handler that forwards messages to the channel."""
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            # Let the SDK parse the payload first.
            chatbot_msg = ChatbotMessage.from_dict(message.data)

            # Fall back to raw payload fields when needed.
            content = ""
            if chatbot_msg.text:
            ...
            # Download file and image payloads.
            file_paths = []
            ...
            # Forward the message without blocking the stream callback.
            ...
            # Return OK so DingTalk does not retry the callback.
...
class DingTalkChannel(BaseChannel):
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # Cache the access token used for sends.
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            handler = AeloonDingTalkHandler(self)
            self._client.register_callback_handler(ChatbotMessage.TOPIC, handler)

            logger.info("DingTalk bot started with Stream Mode")

            # Restart the stream if the SDK exits.
            while self._running:
                try:
    ...
    async def _send_batch_message(
        ...
        if chat_id.startswith("group:"):
            ...
            payload = {
                ...
                "openConversationId": chat_id[6:],  # Drop the "group:" prefix.
                "msgKey": msg_key,
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "Aeloon Reply"},
        )
    ...
    async def _send_media_ref(self, token: str, chat_id: str, media_ref: str) -> bool:
        ...
        if upload_type == "image":
            # sampleImageMsg accepts media_id in photoURL.
            ok = await self._send_batch_message(
                token,
                chat_id,
                "sampleImageMsg",
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        for media_ref in msg.media or []:
            ...
            # Send a visible fallback on upload errors.
            filename = self._guess_filename(media_ref, self._guess_upload_type(media_ref))
    ...
    async def _on_message(
        ...
        """Handle one inbound DingTalk message."""
        try:
    ...
    async def _download_dingtalk_file(
        ...
        """Download a DingTalk file and return the local path."""
        ...
        try:
            ...
            # Save the file under the media directory.
