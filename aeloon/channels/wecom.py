...
# Fallback labels for non-text content.
MSG_TYPE_MAP = {
    "image": "[image]",
    "voice": "[voice]",
    "file": "[file]",
...
class WecomChannel(BaseChannel):
    """WeCom channel using the AI Bot WebSocket API."""
    ...
    def __init__(self, config: Any, bus: MessageBus):
        ...
        # Keep the last frame for each chat so replies can reuse it.
        self._chat_frames: dict[str, Any] = {}

    async def start(self) -> None:
        """Start the WeCom client."""
        if not WECOM_AVAILABLE:
            logger.error("WeCom SDK not installed. Run: pip install aeloon-ai[wecom]")
            return
    ...
    async def _on_mixed_message(self, frame: Any) -> None:
        """Handle a mixed message."""
        await self._process_message(frame, "mixed")

    async def _on_enter_chat(self, frame: Any) -> None:
        """Handle enter_chat."""
        try:
            # Read the frame body from either shape.
    ...
    async def _process_message(self, frame: Any, msg_type: str) -> None:
        """Process one inbound WeCom message."""
        try:
            # Read the frame body from either shape.
            if hasattr(frame, "body"):
                body = frame.body or {}
            ...
            # Ignore unexpected payload types.
            ...
            # Trim the dedup cache.
            while len(self._processed_message_ids) > 1000:
                self._processed_message_ids.popitem(last=False)

            # Read sender info from the SDK payload.
            from_info = body.get("from", {})
            sender_id = (
                from_info.get("userid", "unknown") if isinstance(from_info, dict) else "unknown"
            )

            # Single chats use the sender ID as chat_id.
            ...
            if msg_type == "text":
                ...
                # WeCom voice payloads already include the transcript.
                ...
                # Mixed messages may contain several items.
            ...
            # Save the frame for replies.
            self._chat_frames[chat_id] = frame

            # Put media paths in content for wider model support.
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            # Look up the stored frame for this chat.
