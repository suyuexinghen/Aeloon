...
from aeloon.channels.telegram import TELEGRAM_REPLY_CONTEXT_MAX_LEN, TelegramChannel, TelegramConfig
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="aeloon_test")
...
async def test_start_uses_request_proxy_without_builder_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aeloon.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "aeloon.channels.telegram.Application",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(text="@aeloon_test hi", entities=[mention]), None
    )
    await channel._on_message(
        _make_telegram_update(text="@aeloon_test again", entities=[mention]), None
    )
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    mention = SimpleNamespace(type="mention", offset=0, length=13)
    await channel._on_message(
        _make_telegram_update(caption="@aeloon_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@aeloon_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "aeloon.channels.telegram.get_media_dir",
