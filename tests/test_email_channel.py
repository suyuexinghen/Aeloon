...
from aeloon.channels.email import EmailChannel, EmailConfig
...
def test_fetch_new_messages_parses_unseen_and_marks_seen(monkeypatch) -> None:
    ...
    fake = FakeIMAP()
    monkeypatch.setattr("aeloon.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: fake)
...
async def test_send_uses_smtp_and_reply_subject(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aeloon.channels.email.smtplib.SMTP", _smtp_factory)
