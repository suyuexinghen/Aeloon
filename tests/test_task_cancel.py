...
def _make_loop():
    ...
    with (
        ...
        patch("aeloon.core.agent.loop.SubagentManager") as mock_sub_mgr,
    ):
...
class TestDispatch:
    ...
    async def test_dispatch_processes_and_publishes(self):
        ...
        loop.dispatcher.process_message = AsyncMock(
    ...
    async def test_processing_lock_serializes(self):
        ...
        loop.dispatcher.process_message = mock_process
    ...
    @pytest.mark.asyncio
    async def test_feishu_session_only_keeps_latest_pending_message(self):
        from aeloon.core.bus.events import InboundMessage, OutboundMessage
        ...
        async def mock_process(m, **kwargs):
            ...
            await asyncio.sleep(0.05)
            return OutboundMessage(channel="feishu", chat_id=m.chat_id, content=m.content)

        loop.dispatcher.process_message = mock_process
        runner = asyncio.create_task(loop.dispatcher.run())

        msg1 = InboundMessage(channel="feishu", sender_id="u1", chat_id="c1", content="1")
        msg2 = InboundMessage(channel="feishu", sender_id="u1", chat_id="c1", content="2")
        msg3 = InboundMessage(channel="feishu", sender_id="u1", chat_id="c1", content="3")
...
class TestSubagentCancellation:
    ...
    async def test_subagent_preserves_reasoning_fields_in_tool_turn(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("aeloon.core.agent.tools.registry.ToolRegistry.execute", fake_execute)
