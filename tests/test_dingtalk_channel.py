...
import aeloon.channels.dingtalk as dingtalk_module
from aeloon.channels.dingtalk import AeloonDingTalkHandler, DingTalkChannel, DingTalkConfig
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = AeloonDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/aeloon_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/aeloon_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "aeloon.core.config.paths.get_media_dir",
