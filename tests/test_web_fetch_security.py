...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("aeloon.core.agent.tools._network_safety.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("aeloon.core.agent.tools._network_safety.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with (
        patch("aeloon.core.agent.tools._network_safety.socket.getaddrinfo", _fake_resolve_public),
...
@pytest.mark.asyncio
async def test_web_fetch_timeout_emits_warning_progress():
    tool = WebFetchTool(fetch_timeout_s=0.5, fallback_fetch_timeout_s=0.5)
    warnings: list[str] = []
    ...
    with (
        patch("aeloon.core.agent.tools._network_safety.socket.getaddrinfo", _fake_resolve_public),
        patch("httpx.AsyncClient.get", _timeout_get),
    ):
        result = await tool.execute(url="https://example.com/page", on_progress=on_progress)

    data = json.loads(result)
    assert "Timeout" in data["error"] or "timeout" in data["error"]
    assert len(warnings) == 2
    for w in warnings:
        assert "example.com/page" in w
        assert "timed out" in w.lower() or "timeout" in w.lower()
