"""Tests for aeloon.agent.tools._network_safety — SSRF protection and internal URL detection."""
...
from aeloon.core.agent.tools._network_safety import contains_internal_url, validate_url_target
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch(
        "aeloon.core.agent.tools._network_safety.socket.getaddrinfo",
...
def test_blocks_ipv6_loopback():
    ...
    with patch("aeloon.core.agent.tools._network_safety.socket.getaddrinfo", _resolver):
...
def test_allows_normal_https():
    with patch(
        ...
        ok, err = validate_url_target("https://example.com/aeloon")
        assert ok
